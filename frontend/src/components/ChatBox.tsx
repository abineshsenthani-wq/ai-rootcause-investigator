...
export const ChatBox: React.FC<{ currentDataset: DatasetMeta | null }> = ({ currentDataset }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await sendChatQuestion(currentDataset.id, query);
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[calc(100vh-12rem)] shadow-lg">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          {currentDataset && messages.length > 1 && (
            <button
              ...
              className="text-xs px-2.5 py-1 bg-rose-500/10 border border-rose-500/30 hover:bg-rose-500/20 text-rose-400 rounded-lg flex items-center gap-1 transition-colors font-mono"
            >
          ...
          <span className="text-xs px-2.5 py-1 bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 rounded-full flex items-center gap-1 font-mono">
            <Sparkles className="w-3 h-3" /> Grounded Evidence Active
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4 text-xs">
        ...
        {isSending && (
          <div className="flex gap-3 justify-start items-center text-slate-500 text-xs italic font-mono">
            <span className="w-4 h-4 rounded-full border-2 border-indigo-500 border-t-transparent animate-spin"></span>
            Routing Question & Computing Evidence...
          </div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/60 flex items-center gap-2">
        <input
          ...
          placeholder={
            currentDataset
              ? "Ask a question about metric shifts, anomalies, or segments..."
